...
export function DiagnosticPanel() {
  ...
  const checkSupabaseData = async () => {
    ...
    try {
      console.log(
        "[Diagnostic] Fetching data from Supabase...",
      );
      ...
      if (response.ok) {
        ...
        console.log(
          "[Diagnostic] ✅ Loaded from Supabase:",
          data,
        );
        ...
        if (data.translations?.ko) {
          ...
          ).filter((k) => k.includes("feature"));
          console.log(
            "[Diagnostic] 🎯 Feature keys in DB:",
            featureKeys.length,
          );
          featureKeys.forEach((key) => {
            console.log(
              `  - ${key}: ${data.translations.ko[key]}`,
            );
          });
        }
        ...
        if (data.commonVisibility) {
          const visibilityKeys = Object.keys(
            data.commonVisibility,
          );
          console.log(
            "[Diagnostic] 👁️ Visibility keys in DB:",
            visibilityKeys.length,
          );

          // Feature visible 키만 필터링
          const featureVisibleKeys = visibilityKeys.filter(
            (k) =>
              k.includes("feature") && k.includes("visible"),
          );
          console.log(
            "[Diagnostic] 🎯 Feature visible keys:",
            featureVisibleKeys.length,
          );
          featureVisibleKeys.forEach((key) => {
            console.log(
              `  - ${key}: ${data.commonVisibility[key]}`,
            );
          });
        }
            ...
            {dbData && (
              <div className="mt-4 p-4 bg-white rounded border border-yellow-300 space-y-3">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {/* 번역 일치 여부 체크 */}
                  {Object.keys(dbData.translations?.ko || {})
                    .length !==
                    Object.keys(dbData.translations?.en || {})
                      .length && (
                    <div className="p-2 bg-red-50 border border-red-200 rounded">
                      <div className="text-xs text-red-700">
                        ⚠️ 한국어와 영어 텍스트 개수가 다릅니다.
                        일부 번역이 누락되었을 수 있습니다.
                      </div>
                    </div>
                  )}
                  ...
                  <div className="p-2 bg-orange-50 rounded">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-lg font-bold text-orange-600">
                        {
                          Object.keys(dbData.pageMetadata || {})
                            .length
                        }
                      </div>
                    </div>
